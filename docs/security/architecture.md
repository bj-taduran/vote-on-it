...
| **API Gateway v2** | **Tampering** | Attacker sends a crafted JSON body containing an oversized payload, extra fields, or NoSQL-style injection strings to corrupt DynamoDB expression evaluation. | The handler enforces a **512-byte body size limit** (`REQUEST_TOO_LARGE`) and requires `Content-Type: application/json` before parsing begins. `json.NewDecoder(...).DisallowUnknownFields()` — any extra key returns `400 INVALID_JSON`. Each required field is checked for presence first (`MISSING_*`) then format: `poll_id` against `^poll-[a-z0-9-]{1,50}$`; `option` against enum `{A,B,C,D}`; `voter_id` against a strict UUID v4 regex (version nibble must be `4`, variant nibble in `[89ab]`). All DynamoDB expressions use `ExpressionAttributeValues` and `ExpressionAttributeNames` — raw user input is never interpolated into expression strings. |
